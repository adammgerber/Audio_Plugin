...
void SimpleEQAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    // pass a process spec object to chain that will then pass it to each link in chain
    juce::dsp::ProcessSpec spec;
    // max number of samples to process at one time
...
void SimpleEQAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // processor chain requires processing context passed to it so it can run audio through each link in chain
    // in order to supply processing context we need to create an audio block(wraps buffer)
    juce::dsp::AudioBlock<float> block(buffer);
    
    //use helper func from audio block class to extract channels from buffer
